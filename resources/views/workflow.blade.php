...
?>
<a href="/reset?id={{request('id')}}">reset</a><br>
<hr>
...
@if($workflow->getMarking($m)->has(\App\Workflow\TiAnWorkflow::S_CREATED))
    <ul>
...
@elseif($workflow->getMarking($m)->has(\App\Workflow\TiAnWorkflow::S_X_UPLOADED))
    <ul>
        <li><a href="/submit?id={{request('id')}}">提交给客户</a></li>
        <li><a href="/reject?id={{request('id')}}">未通过</a></li>
        <li><a href="/close?id={{request('id')}}">关闭</a></li>
    </ul>
@elseif($workflow->getMarking($m)->has(\App\Workflow\TiAnWorkflow::S_X_SUBMITTED))
    <ul>
